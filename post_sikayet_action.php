...
<!-- ########################### Sayfa Başladı  ################################################ -->
		 ...
		 <?php
            ...
            $sikayet_details = mysqli_real_escape_string($conn, $_POST["sikayet_details"]);
	    $kimden = $_SESSION['loggedIn_cust_id'];
            $sql0 = "INSERT INTO sikayet (baslik, tarih,sahibi,icerik)
            VALUES('$headline', NOW(),'$kimden','$sikayet_details')";

            ?>

            <?php
            if (($conn->query($sql0) === TRUE)) { ?>
                <div class="alert alert-success" role="alert"><?php echo "Şikayetiniz Alınmıştır!\n"; ?></div>
            <?php
            } else { ?>
			...
			<a class="btn btn-primary" href="/post_sikayet.php" class="button">Geri</a>
        </div>
